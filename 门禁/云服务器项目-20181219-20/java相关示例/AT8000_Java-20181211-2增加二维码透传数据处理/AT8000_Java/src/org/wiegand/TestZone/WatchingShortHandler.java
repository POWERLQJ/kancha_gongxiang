...
import org.wiegand.at8000.wgControllerInfo;
    ...
    public class WatchingShortHandler extends IoHandlerAdapter {
       ...
       public WatchingShortHandler(Queue<byte[]> queue) {
        ...
        public WatchingShortHandler(Queue<byte[]> queue,Queue<byte[]> UDPQueue4Mobile,Queue<IoSession> RemoteEndPointQueue4Mobile
       ...
       /**
        ...
        public void messageReceived(IoSession session, Object message)
    		...
    		if (io.hasRemaining())
    		{
    			...
    			{
     				 if ((validBytes.length == 64) && ((validBytes[0] == 0x37)) && ((validBytes[1]) == 0x40)) //2015-06-24 15:07:51 表示手机专用指令 手机专用指令 且是远程开门
                     {
                         //UDPQueue4Mobile
     			    ...
     			    else if ((validBytes.length == 64) && ((validBytes[0] == 0x17)) && ((validBytes[1]) == 0x40) && ((validBytes[43] & 0xf0)==0x70)) //2018-02-12 17:21:57流水号为0x70000000-0x7FFFFFFF 2015-06-24 15:12:40 控制器的手机回复指令
                    {
		            ...
		            if (iget < 0)
		            {
						...
						WatchingShortHandler.arrRecordIndex.add((int) recordIndexGet); //2017-09-07 11:23:50 保存新值
