...
public class WgUdpCommShort4CloudClient { //短报文协议

	public static final int  WGPacketSize = 64;             //报文长度
	public static final int  WGPacket1024Size = 1024;             //1024报文长度
//	public static final byte Type = 0x17; //2015-04-30 08:50:29 0x19;					//类型
	public static final byte Type = 0x37; //2019-05-19 08:48:20 发送给云服务器的指令 //2015-04-30 08:50:29 0x19;					//类型
//	public static final int  ControllerPort = 60000;        //控制器端口
	public static final int  CloudServerPort = 61005;        //控制器端口
    ...
    {
        _Global_xid++;
        if (_Global_xid >= 0x7fffffff)//手机远程开门时使用流水号
		{
        	_Global_xid = 0x70000001; //2018-02-13 19:06:22
		}
        else if (_Global_xid<0x70000001)
        {
        	_Global_xid = 0x70000001; //2018-02-13 19:06:22
        }
    ...
    static long getXidOfCommand(byte[] cmd) //获取指令中的xid
    {
	   ...
	   public WgUdpCommShort4CloudClient()
		{
			Reset();
		}
	...
	{
		//2019-05-19 08:48:45 CommOpen(ip,ControllerPort);  //2013-11-06 14:16:52 默认是60000
		CommOpen(ip,CloudServerPort);  //2013-11-06 14:16:52 默认是60000
	}
    ...
    public  byte[] getInfo( long sn, byte[] command) 
    {
		...
		synchronized(queue)//2019-05-19 17:07:34 先清空接收缓存
 		{
			 queue.clear();
 		}
         while ((tries--) > 0)  
